...
Camera::Camera(): camera_mode(FREE), move_camera(false),
                  field_of_view(45), max_pitch_rate(0.3f), max_heading_rate(0.3f),
                  viewport_x(0), viewport_y(0), camera_scale(0.1f), camera_heading(0), camera_pitch(0),
                  near_clip(0.1), far_clip(100000) {
...
void Camera::Move2D(int x, int y) {
	...
	if (move_camera) {
        float rate = 0.01f; // 0.08
		ChangeHeading(rate * mouse_delta.x);
		ChangePitch(rate * mouse_delta.y);
	}
